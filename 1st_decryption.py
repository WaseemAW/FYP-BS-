...
class Window:  
    def __init__(self,root):
        ...
        m_title=Label(Manage_frame,text='Decryption First Stage',bg='#04444a',fg='white' ,font=('time new roman',30,'bold')).place(x=100,y=10,width=1000)
        ...
        RGB_btn=Button(btn_frame,text='Click for First Decryption',command=self.TSED,width=19).grid(row=0,column=1,padx=90)
        TSED_btn=Button(btn_frame,text='Click for TSED',command=self.final,width=21).grid(row=0,column=2,padx=15)
        #save_btn=Button(btn_frame,command=self.text_gui2,text='Next',width=12).grid(row=0,column=3,padx=15)
        
    
    def final(self):
        #self.root.destroy()
        call(["python", "2nd_decryption.py"])  
